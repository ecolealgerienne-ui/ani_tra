...
class MedicalProductsTable extends Table {
  ...
  TextColumn get activeIngredient =>
      text().nullable().named('active_ingredient')();
  ...
  // Withdrawal periods
  IntColumn get withdrawalPeriodMeat =>
      integer().named('withdrawal_period_meat')();
  IntColumn get withdrawalPeriodMilk =>
      integer().named('withdrawal_period_milk')();
  ...
  TextColumn get storageConditions =>
      text().nullable().named('storage_conditions')();
  ...
  TextColumn get targetSpecies =>
      text().withDefault(const Constant('')).named('target_species')();

  // Treatment-specific fields
  IntColumn get standardCureDays =>
      integer().nullable().named('standard_cure_days')();
  TextColumn get administrationFrequency =>
      text().nullable().named('administration_frequency')();
  TextColumn get dosageFormula => text().nullable().named('dosage_formula')();

  // Vaccine-specific fields
  TextColumn get vaccinationProtocol =>
      text().nullable().named('vaccination_protocol')();

  // List<int> stored as comma-separated TEXT ('21,365')
  TextColumn get reminderDays => text().nullable().named('reminder_days')();

  TextColumn get defaultAdministrationRoute =>
      text().nullable().named('default_administration_route')();
  TextColumn get defaultInjectionSite =>
      text().nullable().named('default_injection_site')();
  ...
  /// Custom statements with FK + 4 performance indexes
  List<String> get customStatements => [
        // Index for farmId filtering
        'CREATE INDEX IF NOT EXISTS idx_medical_products_farmid ON medical_products(farm_id);',

        // Index for farmId + isActive filtering
        'CREATE INDEX IF NOT EXISTS idx_medical_products_farm_active ON medical_products(farm_id, is_active);',

        // Index for stock queries
        'CREATE INDEX IF NOT EXISTS idx_medical_products_stock ON medical_products(farm_id, current_stock, min_stock);',
